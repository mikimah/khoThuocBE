...
const donhangController = {
    ...
    try {
      ...
      if (cacheData) {
        return response.ok(
          res,
          cacheData,
          "Lấy danh sách đơn hàng thành công (từ cache)",
        );
      }
  ...
  getDonHangCuaToi: async (req, res, next) => {
    try {
      ...
      if (!mataikhoan)
        return response.unauthorized(res, "Không xác thực được tài khoản");
      ...
      if (
        ...
        if (!soHoaDon) {
          return response.badRequest(
            res,
            "Số hóa đơn GTGT là bắt buộc đối với Phiếu Nhập!",
          );
        }
      }
  ...
  traCuuCongKhai: async (req, res, next) => {
    try {
      ...
      // Bỏ tìm theo SĐT
      const masterRows = await donhangModel.getPublicByTrackingOrPhone(
        mavandon3pl,
        null,
      );
  ...
  updateTrangThai: async (req, res, next) => {
    try {
      ...
      if (!trangthai)
        return response.badRequest(res, "Thiếu dữ liệu trạng thái");

      const result = await donhangModel.updateStatusWithBusinessLogic(
        id,
        trangthai,
      );
      ...
      const userId = await donhangModel.getDoiTacByMaDH(id);
      if (userId.length === 0)
        return response.notFound(res, "Không tìm thấy đối tác");
      let soLanGiaoDich = await doitacModel.getSoLanGiaoDichThanhCong(
        userId[0].madoitac,
      );
      soLanGiaoDich = soLanGiaoDich[0]?.solangiaodich_thanhcong || 0;

      await doitacModel.updateLanGiaoDichThanhCong(
        userId[0].madoitac,
        soLanGiaoDich + 1,
      );

      await redisFunc.deleteCache(cacheKey); // Xóa cache sau khi cập nhật
      await redisFunc.deleteCache(`doitac`);
