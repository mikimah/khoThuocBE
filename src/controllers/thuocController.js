...
const ThuocController = {
  getAll: async (req, res, next) => {
    try {
      ...
      if (cacheData) {
        return response.ok(res, cacheData, "Lấy danh sách thành công");
      }
  ...
  getById: async (req, res, next) => {
    try {
      ...
      if (data.length === 0)
        return response.notFound(res, "Không tìm thấy thuốc này");
  ...
  create: async (req, res, next) => {
    try {
      ...
      return response.created(
        res,
        { id: result.insertId, ...newThuocData },
  ...
  update: async (req, res, next) => {
    try {
      ...
      if (updateData.tenthuoc) {
        ...
        const isExist = await ThuocModel.checkDuplicate(
          updateData.tenthuoc,
          id,
        );
        if (isExist) {
      ...
      if (result.affectedRows === 0)
        return response.notFound(res, "Không tìm thấy thuốc để cập nhật");
  ...
  delete: async (req, res, next) => {
    try {
      ...
      if (result.affectedRows === 0)
        return response.notFound(res, "Không tìm thấy thuốc để xóa");
